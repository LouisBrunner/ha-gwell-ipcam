...
from .api import PTZ_DIRECTIONS, map_ptz_direction
...
class GwellIPCamCamera(GwellIPCamEntity[GwellIPCamCoordinator], Camera):
    ...
    async def async_camera_image(
        ...
        """
        Grab one live frame and JPEG-encode it (replaces rtsp-fixer's thumbnail endpoint).

        The camera is only powered on when protecting the house, so this is expected to fail
        most of the time -- rather than erroring or timing out per call (dashboards poll this
        continuously), fall straight back to the last-known-good frame with the current error
        overlaid, without even attempting a connection once we already know we're offline.
        """
        ...
        if session.online:
            ...
            jpeg = await self.hass.async_add_executor_job(_grab_snapshot_jpeg, url) if url else None
            if jpeg is not None:
                LOGGER.debug("Snapshot for %s succeeded (%d bytes)", self.entity_id, len(jpeg))
                client.frame_cache.update(jpeg)
                return jpeg
            message = "failed to read the live stream"
        else:
            message = str(session.last_error) if session.last_error else "camera offline"
        LOGGER.debug("Snapshot for %s falling back to the error frame: %s", self.entity_id, message)
        jpeg, _width, _height = await self.hass.async_add_executor_job(client.frame_cache.render_error, message)
        return jpeg
...
def _grab_snapshot_jpeg(url: str) -> bytes | None:
    ...
    try:
        with av.open(url, options={"rtsp_transport": "tcp"}, timeout=5.0) as container:
